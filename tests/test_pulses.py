...
def test_square_pulse():
    p = SquarePulse(duration=20.0, amp=0.5)
    t, c_wave = p.sample(dt=1.0)
    assert len(t) == 21
...
def test_flattop_pulse():
    p = FlatTopPulse(duration=50.0, amp=1.0, ramp_time=10.0, ramp_type="cosine")
    t, c_wave = p.sample(dt=0.5)
...
def test_drag_quadrature():
    ...
    p_no_drag = GaussianPulse(duration=40.0, amp=1.0, drag=0.0)
    p_drag = GaussianPulse(duration=40.0, amp=1.0, drag=0.5)

    _, wave_no_drag = p_no_drag.sample(dt=0.5)
    _, wave_drag = p_drag.sample(dt=0.5)
...
def test_pulse_fft_and_spectral_leakage():
    ...
    cutoff = 0.1  # 100 MHz
    leak_square = spectral_leakage(p_square, cutoff_freq=cutoff)
    leak_cosine = spectral_leakage(p_cosine, cutoff_freq=cutoff)
...
def test_pulse_scaling():
    p = GaussianPulse(duration=30.0, amp=0.2)
    p_scaled = p * 3.0
