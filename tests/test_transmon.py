"""Tests for sqpulse.models.Transmon."""
...
def test_transmon_operators():
    q = Transmon("q0", f_q=5.0, alpha=-0.25, levels=4)
...
def test_transmon_collapse_operators():
    q_ideal = Transmon("q0", f_q=5.0, levels=3)
    assert len(q_ideal.c_ops()) == 0

    q_lossy = Transmon("q0", f_q=5.0, levels=3, t1=10000.0, t2=8000.0)
    c_ops = q_lossy.c_ops()
